...
def get_distance_time_matrix(locations):
    ...
    if os.path.exists(dist_file) and os.path.exists(time_file):
        ...
        if n <= 10:
            ...
            dist, times = Route_Matrix_Api.get_distance_time_matrix_routes(locations)
...
if __name__ == "__main__":
    ...
    # ------------------------------------------------
    for agent_id, assigned_deliveries in final_result.items():
        ...
        locations = [
            (d["Drop_Latitude"], d["Drop_Longitude"])
            for d in assigned_deliveries
        ]
